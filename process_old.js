/* 
    0. Monitor each url for trademark.
    -> Bootsrap
	-> (Check auth status).
		-> If Logged in: 
			(Check token validity)
			-> If token valid, get subscription info.
				-> If subscription valid:
					-> scrap the page and generate data
					-> Send data to server to calculate
					-> Receive calculated data
					-> Update app state with data 
				-> Else, set subs to false in storage.
			-> Else, log user out.
		-> Do no initiate.
*/


let trigger = true; // true only when logged in
